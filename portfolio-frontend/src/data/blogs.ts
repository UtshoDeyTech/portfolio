...
// API Endpoints - Dynamically determined based on execution context
const getApiEndpoint = (path: string) => `${getApiUrl()}${path}`;
const BLOG_POSTS_API = () => getApiEndpoint('/api/blog-posts/');
const BLOGS_API = () => getApiEndpoint('/api/blogs/');
